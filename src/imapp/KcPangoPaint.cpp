...
void KcPangoPaint::beginPaint()
{
    ...
    cairoSurf_->clear({ 0, 0, 0, 1 });
}


void KcPangoPaint::endPaint()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, cairoSurf_->width(), cairoSurf_->height(), 0, GL_RGB, GL_UNSIGNED_BYTE, data);
    glBegin(GL_QUADS);
